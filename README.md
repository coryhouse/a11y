...
- Role is a last resort. Again, prefer a specific tag first. `h1` has a role="heading. `button` has role="button". `footer` has role="complimentary", `header` has role="banner" (when not nested under <aside>, <article>, <main>, <nav>, or <section>.) (and yes, `header`/`footer` can be used elsewhere in the page to mark up headers footers for sections too. Notable roles that don't have a tag equivalent: [feed](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Roles/Feed_Role), 
