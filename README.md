...
- Role is a last resort. Again, prefer a specific tag first. `button` has role="button". `header` has role="banner" (when not nested under <aside>, <article>, <main>, <nav>, or <section>.) (and yes, `header`/`footer` can be used elsewhere in the page to mark up headers footers for sections too.
